...
function calcularLogicaSemaforo(pctKm, pctHora, pctNota) {
  ...
  function corGanho(percentual) {
    if (percentual < 50) return "vermelho";
    if (percentual < 100) return "amarelo";
    return "verde";
  }

  function corNota(percentual) {
    if (percentual < 80) return "vermelho";
    if (percentual < 100) return "amarelo";
    return "verde";
  }

  const corKm = corGanho(km);
  const corHora = corGanho(hora);
  const corNotaFinal = corNota(nota);
  ...
  if (
    corKm === "vermelho" ||
    corHora === "vermelho" ||
    corNotaFinal === "vermelho"
  ) {
    decisao = "RECUSAR";
  } else if (
    corKm === "amarelo" ||
    corHora === "amarelo" ||
    corNotaFinal === "amarelo"
  ) {
    decisao = "ANALISAR";
  }
...
function gerarPacoteCopiloto(texto) {
  ...
  const statusKm =
    avaliarRegra(indicadores.lucroPorKm, regras.metaKm);

  const statusHora =
    avaliarRegra(indicadores.lucroPorHora, regras.metaHora);

  const statusNota =
    avaliarNota(dados.nota, regras.notaMinima);
  ...
  const decisaoBase =
    decisaoFinal(statusKm, statusHora, statusNota);

  const analisePro =
    aplicarModoPro(decisaoBase, dados, indicadores, regras);
  ...
  const scoreAdaptativo =
    calcularScoreAdaptativo(
      dados,
      indicadores,
      {
        km: statusKm,
